...
def create_dataset(image_shape=64, n_set='train', original_image_shape=256, 
    ...
    """
    print("Entering dataset function of utils.py")
    if n_set == 'train':
        number_set = 3
        name_set = 'Training'
    elif n_set == 'test':
        ...
        name_set = 'Online'         
    # Create dataset
    series = json.load(open('/content/drive/My Drive/Medical-Image-Analysis-master/series_case.json'))[n_set]
    ...
    for case, serie in series.items():
        ...
        '''
        image_path_base='/content/drive/My Drive/Medical-Image-Analysis-master/Data2/newdataset_updated/'+'%s/%s'%(case, serie)
        contour_path_base='/content/drive/My Drive/Medical-Image-Analysis-master/manual_contours/manual_contours'+'/%s/contours-manual/IRCCI-expert/' % ( case)
        ...
        for c in contours_list_series:
            ...
            image_part = pydicom.read_file(image_path).pixel_array  
